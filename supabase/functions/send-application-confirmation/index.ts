...
function buildEmailHtml(params: { firstName: string; statusUrl: string }): string {
  ...
  <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
    ...
    <p>vielen Dank für deine Bewerbung bei Kanzleistelle24! Wir haben sie gerade erhalten und schauen sie uns in Ruhe an.</p>
    <p>Du musst dich um nichts kümmern – sobald sich der Status ändert, kannst du das jederzeit über deinen persönlichen Link nachsehen.</p>
    <div style="background:#f5f5f0;border-radius:12px;padding:18px 20px;margin:24px 0;display:flex;align-items:center;gap:16px;">
      ...
      <div>
        ...
        <p style="font-size:13px;line-height:1.5;color:#333;margin:0;">
          „Ich kümmere mich persönlich um deine Bewerbung und melde mich, sobald es Neuigkeiten gibt."
        </p>
      </div>
    </div>
...
serve(async (req) => {
  ...
  try {
    const { applicationId, firstName, email, statusToken } = await req.json();

    if (!email || !statusToken) {
      return new Response(JSON.stringify({ error: "email und statusToken sind erforderlich." }), {
        status: 400,
        headers: { ...corsHeaders, "Content-Type": "application/json" },
      });
    }

    console.log("[send-application-confirmation] applicationId:", applicationId, "email:", email);

    const statusUrl = `https://kanzleistelle24.de/bewerbung-status?token=${statusToken}`;
    const htmlContent = buildEmailHtml({ firstName: firstName || "", statusUrl });
    ...
    const mailRes = await fetch("https://api.brevo.com/v3/smtp/email", {
      ...
      body: JSON.stringify({
        ...
        subject: "Deine Bewerbung ist bei uns eingegangen! 🎉",
        htmlContent,
      }),
    });
    ...
    console.error("send-application-confirmation error:", e);
