...
const ApplicantProfileEditor = ({ application, userId }: ApplicantProfileEditorProps) => {
  ...
  const saveMutation = useMutation({
    ...
    onSuccess: async (_data, variables) => {
      ...
      if (variables.email) {
        try {
          const tpl = buildApplicantConfirmationEmail({ applicantName: displayName });
          const { error: emailError } = await supabase.functions.invoke("send-contact-email", {
            body: { to_email: variables.email, to_name: displayName, subject: tpl.subject, html: tpl.html },
          });
          if (emailError) {
            console.warn("[profile-save] Applicant email failed:", emailError);
            toast.info("E-Mail-Bestätigung konnte nicht gesendet werden, Profil wurde aber gespeichert.");
          }
        } catch (e) { console.warn("[profile-save] Applicant email error:", e); }
      }

      // 2) Profile-update notification to employer (Template B)
      try {
        const jobTitle = application?.jobs?.title || application?.job_title || "Stelle";
        const companyId = application?.company_id;
        if (companyId) {
          const { data: company } = await supabase.from("companies").select("user_id, name").eq("id", companyId).maybeSingle();
          if (company?.user_id) {
            const { data: profile } = await supabase.from("profiles").select("email").eq("id", company.user_id).maybeSingle();
            if (profile?.email) {
              const tpl = buildProfileUpdateEmail({
                applicantName: displayName,
                jobTitle,
                companyName: company.name || "Kanzlei",
              });
              await supabase.functions.invoke("send-contact-email", {
                body: { to_email: profile.email, to_name: company.name || "Kanzlei", subject: tpl.subject, html: tpl.html },
              });
            }
          }
        }
      } catch (e) { console.warn("[profile-save] Employer notification error:", e); }
    },
    onError: (error: any) => {
